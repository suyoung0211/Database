...
CREATE TABLE tbl_javaword (
		idx number(10),
		english varchar2(100),  -- varchar2 가변길이. 최대 100 byte
		korean varchar2(500),   -- varchar2 가변길이. 최대 500 byte
		step char(1)            -- char     고정길이. 반드시 1 byte
);
...
/* 
    insert, update, delete 는 테이블의 데이터를 추가 / 수정 / 삭제 하는 변경작업입니다.ALTER
...
-- 컬럼 값에 적용하여 조회하기
-- 함수 실행 결과로 새로운 칼럼이 만들어 집니다. as 를 사용하여 컬럼의 별칭을 정합니다.
SELECT UPPER(english) as "대문자", LENGTH(english) as "길이"
	FROM TBL_JAVAWORD ;

-- UPPER 은 컬럼 값을 대문자로 변환해서 출력
-- LENGTH 은 컬럼의 문자열 길이를 계산해서 출력
