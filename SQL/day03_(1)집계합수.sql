/*
    집계(통계) 함수 : count, sum, avg, max, min, 표준편차, 분산
                    ...
                    🌟 tvl_car_co2 테이블에서는 합계, 평균, 최대, 최소값은 number 칼럼인
                     vloume, weight, co2 를ㄹ 대상으로 테스트 할 수 있습니다.
...
--- Null 값이 있는 WEIGHT 컬럼 : 나누기 9 => sum() / count(weight)
SELECT AVG(WEIGHT)
FROM TBL_CAR_CO2;

--- 2) 평균 Null 값은 0 으로 바꾸기 : 나누기 10 => sum() / count(*)
SELECT WEIGHT
FROM TBL_CAR_CO2;
...
SELECT
    AVG(CASE WHEN eWEIGHT >= 1000 THEN WEIGHT END), -- when <조건식> then <값>
    AVG(CASE WHEN eWEIGHT < 1000 THEN WEIGHT END)
FROM TBL_CAR_CO2;


SELECT
    SUM(CASE WHEN eWEIGHT >= 1000 THEN 1 END), -- when <조건식> then <값>
    SUM(CASE WHEN eWEIGHT < 1000 THEN 1 END)
FROM TBL_CAR_CO2;
