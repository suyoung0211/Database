-- DML : select, insert, updata, delete
--      테이블의 행을 대상으로 하는 명령어
...
-- insert, update, delete 는 변경사항에 대해 바로 테이블에 반영(commit) 하지않고
...
-- commit 하기전 까지는 변견된 데이터를 확인할 수 없습니다.(반영 x)
...
-- 테이블에 직접 가서 데이터 탭을 확인하면 행 추가 안되어 있음(세션이 다름)
